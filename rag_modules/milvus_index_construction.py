...
class MilvusIndexConstructionModule:
    ...
    def __init__(self, 
        ...
        Args:
            host: Milvus服务器地址（uri未设置时使用）
            port: Milvus服务器端口（uri未设置时使用）
            collection_name: 集合名称
            dimension: 向量维度
            model_name: 嵌入模型名称
            uri: Milvus连接URI，支持：
                 - Milvus Lite（本地文件，无需服务器）: "./milvus_data.db"
                 - Milvus服务器: "http://localhost:19530"
        """
        self.host = host
        self.port = port
        ...
        self.uri = uri if uri else f"http://{host}:{port}"
    ...
    def _setup_client(self):
        """初始化Milvus客户端（支持 Lite 本地文件模式和服务器模式）"""
        try:
            self.client = MilvusClient(uri=self.uri)
            if self.uri.endswith(".db"):
                logger.info(f"Milvus Lite 模式（本地文件）: {self.uri}")
            else:
                logger.info(f"已连接到Milvus服务器: {self.uri}")
