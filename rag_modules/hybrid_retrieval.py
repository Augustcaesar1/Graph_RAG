...
class HybridRetrievalModule:
    ...
    def __init__(self, config, index_module, data_module, llm_client):
        self.config = config
    ...
    def _build_graph_index(self):
        ...
        try:
            # 获取图数据（兼容：东周列国数据模块没有 cooking_steps）
            recipes = self.data_module.recipes
            ingredients = self.data_module.ingredients
            cooking_steps = getattr(self.data_module, "cooking_steps", [])
    ...
    def _extract_relationships_from_graph(self) -> List[Tuple[str, str, str]]:
        ...
        try:
            with self.driver.session() as session:
                ...
                RETURN source.name as source_id, type(r) as relation_type, target.name as target_id
                LIMIT 1000
                """
    ...
    def extract_query_keywords(self, query: str) -> Tuple[List[str], List[str]]:
        ...
        """
        prompt = f"""
        作为东周列国历史知识助手，请分析以下查询并提取关键词，分为两个层次：

        查询：{query}

        提取规则：
        1. 实体级关键词：具体的人物、国家、战争/事件名称等实体
           - 例如：齐桓公、管仲、孙膑、庞涓、齐国、晋国、长平之战

        2. 主题级关键词：抽象概念、关系主题、历史时期、政治/军事主题等
           - 例如：春秋、战国、变法、合纵连横、盟友、敌人、君臣、师生
           - 排除动作词：介绍、讲讲、是什么、有哪些等

        示例：
        查询："孙膑和庞涓是什么关系"
        {{
            "entity_keywords": ["孙膑", "庞涓"],
            "topic_keywords": ["关系", "对手"]
        }}

        查询："长平之战有哪些人参与"
        {{
            "entity_keywords": ["长平之战"],
            "topic_keywords": ["参战人物", "主攻", "主守"]
        }}
    ...
    def _neo4j_topic_level_search(self, keywords: List[str], limit: int) -> List[RetrievalResult]:
        ...
        try:
            with self.driver.session() as session:
                ...
                UNWIND $keywords as keyword
                MATCH (n)
                WHERE (n.state CONTAINS keyword)
                   OR (n.is_king CONTAINS keyword)
                   OR (n.location CONTAINS keyword)
                   OR (n.attacker CONTAINS keyword)
                   OR (n.defender CONTAINS keyword)
                   OR (n.result CONTAINS keyword)
                WITH n, keyword
                OPTIONAL MATCH (n)-[r]->(m)
                WITH n, keyword, collect(m.name)[0..3] as neighbors
                RETURN
                    n.name as node_id,
                    n.name as name,
                    n.state as category,
    ...
    def vector_search_enhanced(self, query: str, top_k: int = 5) -> List[Document]:
        ...
        try:
            ...
            enhanced_docs = []
            for result, score in vector_docs_with_scores:
                # 从Document对象获取信息
                ...
                # 调试：打印向量得分
                logger.debug(f"向量检索得分: {recipe_name} = {score}")
    ...
    def _get_node_neighbors(self, node_id: str, max_neighbors: int = 3) -> List[str]:
        ...
        try:
            with self.driver.session() as session:
                query = """
                MATCH (n {name: $node_id})-[r]-(neighbor)
